                    ...
                    <div class="table-box">
                        ...
                        <div class="pull-left">
                            <div class="form-group form-inline">
                                <div class="btn-group">
                                    <a href="${pageContext.request.contextPath}/health/toHealyCardRecord.action"> <button type="button" class="btn btn-default" title="新建"><i class="fa fa-file-o"></i> 新建</button></a>
                                    <!--对应学生信息的增加-->
                                </div>
                            </div>
                        </div>
...
<%--                            </div>--%>
                        ...
                        <table id="dataList" class="table table-bordered table-striped table-hover dataTable">
                            ...
                            <tr>
                                ...
                                <th class="sorting_asc sorting_asc_disabled">药品名</th>
                                <th class="sorting_desc sorting_desc_disabled">是否需转至医院</th>
                                <th class="text-center">操作</th>
                            </tr>
                            </thead>
                            <tbody>
                            <c:forEach items="${healthpageBean.list}" var="record">
                                <tr>
                                    <td><input name="ids" type="checkbox"></td>
                                    <td>${record.id}</td>
                                    <td>${record.remarkNum}</td>
                                    <td>${record.studentName}</td>
                                    <td>${record.sex}</td>
                                    <td>${record.stime}</td>
                                    <td>${record.address}</td>
                                    <td>${record.medicineName}</td>
                                    <td>${record.ismoveToHosp}</td>

                                    <td class="text-center">
                                        <!--对应后端controller信息的删除的RequestMapping名字-->
                                        <a href="${pageContext.request.contextPath}/health/delteByOne.action?id=${record.id}"><button type="button"  class="btn bg-olive btn-xs">删除</button></a>

                                        <a href="${pageContext.request.contextPath}/health/updateByOne.action?healthCardRecordId=${record.id}"><button type="button" class="btn bg-olive btn-xs">编辑</button></a>
                                    </td>
                                </tr>
                            </c:forEach>
                            </tbody>
                ...
                <div class="box-footer">
                    <div class="pull-left">
                        <div class="form-group form-inline">
                            第 ${healthpageBean.currentPage } / ${healthpageBean.totalPage }
                            &nbsp;&nbsp;
                            每页显示${healthpageBean.pageSize }条  &nbsp;&nbsp;&nbsp;
                            总计${healthpageBean.totalSize }条记录 &nbsp;&nbsp;&nbsp;
                        </div>
                    </div>

                    <div class="box-tools pull-right">
                        <ul class="pagination">
                            <c:if test="${healthpageBean.currentPage !=1 }">
                                <li><a href="${pageContext.request.contextPath}/health/healthcardrecord_Page.action?currentPage=1">首页</a></li>
                                <li><a href="${pageContext.request.contextPath}/health/healthcardrecord_Page.action?currentPage=${healthpageBean.currentPage-1 }">上一页</a></li>
                            </c:if>

                            <c:forEach begin="1" end="${healthpageBean.totalPage }" var="i">
                                <c:if test="${healthpageBean.currentPage == i }">
                                    <li><a href="${pageContext.request.contextPath}/health/healthcardrecord_Page.action?currentPage=${i}">${i }</a></li>
                                </c:if>
                                <c:if test="${healthpageBean.currentPage != i }">
                                    <li><a href="${pageContext.request.contextPath}/health/healthcardrecord_Page.action?currentPage=${i}">${i }</a></li>
                                </c:if>
                            ...
                            <c:if test="${healthpageBean.currentPage !=healthpageBean.totalPage }">
                                <li><a href="${pageContext.request.contextPath}/health/healthcardrecord_Page.action?currentPage=${healthpageBean.currentPage+1 }">下一页</a></li>
                                <li><a href="${pageContext.request.contextPath}/health/healthcardrecord_Page.action?currentPage=${healthpageBean.totalPage }">尾页</a></li>
                            </c:if>
                        </ul>
                    </div>
